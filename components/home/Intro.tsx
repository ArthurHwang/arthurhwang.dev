// import { Fragment } from "react";
// import { Query } from "react-apollo";
// import LogoNodejs from "react-ionicons/lib/LogoNodejs";
// import { NextPage } from "next";
import styled from "styled-components";
// import Head from "next/head";
// import gql from "graphql-tag";
// import LogoNodejs from "react-ionicons/lib/LogoNodejs";
// import Fonts from "../util/Fonts";
// import { withApollo } from "../lib/apollo";
// import { useLayoutEffect } from "react";
import Link from "next/link";

import {
  FaGithubSquare,
  FaLinkedin,
  FaFacebookSquare,
  FaFileCode,
  // FaRegCopyright,
  FaTwitterSquare
} from "react-icons/fa";
import { IoMdMail } from "react-icons/io";

export const Intro: React.FC<any> = () => {
  return (
    <StyledIntro>
      <InnerLeft>
        <h2>I like to build things with code.</h2>
        <p></p>
        <p>
          As a natural tinkerer, I love to dig out the "why" of things. Being a
          self taught developer has taught me to not fear what I don't know,
          instead tackle it with sheer determination and an unwavering will.
          Having the privilege everyday to produce elegant and performant
          software solutions makes me happy.
        </p>
        <p>
          I am drawn to creating performance driven websites that are fully
          responsive using best practices that work on a wide range of devices.
          I take tremendous care to ensure that the project I am working on is
          not only scalable but also maintainable. Please visit my{" "}
          <Link href="/blog">
            <a className="link">Blog</a>
          </Link>
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Quidem
          suscipit ratione aliquam eaque minima delectus neque voluptates
          quisquam tempora eveniet nostrum molestias excepturi dolorem, ab
          maxime quas praesentium commodi voluptatum!
        </p>
      </InnerLeft>

      <InnerRight>
        <h2>Links</h2>

        <div className="badges">
          <a
            title="Github"
            href="https://github.com/ArthurHwang"
            className="link"
          >
            <FaGithubSquare className="icon" /> Github
          </a>
          <a
            title="LinkedIn"
            href="https://www.linkedin.com/in/arthur-hwang"
            className="link"
          >
            <FaLinkedin className="icon" /> LinkedIn
          </a>
          <a
            title="Facebook"
            href="https://www.facebook.com/arthur.hwang.9"
            className="link"
          >
            <FaFacebookSquare className="icon" /> Facebook
          </a>
          <a title="Twitter" href="https://www.twitter.com/" className="link">
            <FaTwitterSquare className="icon" /> Twitter
          </a>
          <a title="Email" href="mailto:mail@arthurhwang.dev" className="link">
            <IoMdMail
              style={{ position: "relative", top: "6px" }}
              className="icon"
            />{" "}
            Email
          </a>
          <a title="Resume" href="mailto:mail@arthurhwang.dev" className="link">
            <FaFileCode
              style={{ position: "relative", top: "3px" }}
              className="icon"
            />{" "}
            CV
          </a>
        </div>
      </InnerRight>

      {/* <Query query={GET_REPOSITORIES_OF_CURRENT_USER}>
          {({ data, loading }: Query) => {
            if (loading || !data) {
              return <div>Loading ...</div>;
            }
            return <div>{JSON.stringify(data)}</div>;
          }}
        </Query> */}
    </StyledIntro>
  );
};

const InnerLeft = styled("div")`
  padding-left: 12rem;
`;

const InnerRight = styled("div")`
  .badges {
    & a {
      margin-bottom: 1rem;
    }
    display: flex;
    flex-direction: column;

    .icon {
      position: relative;
      top: 4px;
      font-size: 3rem;
    }
  }
`;

const StyledIntro = styled("div")`
  display: grid;
  grid-template-columns: 1fr 0.3fr;
  grid-gap: 4rem;

  @media (max-width: 490px) {
    grid-template-columns: initial;
    grid-template-rows: 1fr auto;
    grid-gap: initial;
  }
`;
